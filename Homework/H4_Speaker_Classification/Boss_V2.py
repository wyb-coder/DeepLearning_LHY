...
class SelfAttentionPooling(nn.Module):
    """
    升级为 Attentive Statistics Pooling (ASP)
    不仅计算加权均值，还计算加权标准差
    """
    ...
    def forward(self, x):
        ...
        w = self.attention(h)  # (Batch, Length, 1)
        w = torch.softmax(w, dim=1)  # (Batch, Length, 1)

        # 2. 计算加权均值 (Mean)
        mu = torch.sum(x * w, dim=1)  # (Batch, Dim)

        # 3. 计算加权标准差 (Std)
        # 核心公式: sqrt( sum( w * (x - mu)^2 ) )
        # 需要利用广播机制
        rh = torch.sqrt(torch.sum(w * (x - mu.unsqueeze(1)) ** 2, dim=1) + 1e-6)

        # 4. 拼接
        return torch.cat((mu, rh), dim=1)  # (Batch, Dim * 2)
