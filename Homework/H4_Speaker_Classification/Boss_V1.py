...
class Classifier(nn.Module):
    def __init__(self, d_model=256, n_spks=600, dropout=0.15):
        super().__init__()
    ...
    def forward(self, mels, labels=None):
        ...
        lengths = torch.ones(out.size(0), device=out.device, dtype=torch.long) * out.size(1)
        out, _ = self.conformer(out, lengths=lengths)

        stats = self.sap(out)  # 输出 (Batch, Dim)

        embedding = self.pred_layer(stats)
...
class SelfAttentionPooling(nn.Module):
    """
    实现论文: https://arxiv.org/pdf/2008.01077v1.pdf (SAP 机制)
    """

    def __init__(self, input_dim):
        super().__init__()
        # W: 用于把特征投影到一个标量分数 (Score)
        # 这里是一个简化的实现，也可以用 W1(dim->hidden) -> Tanh -> W2(hidden->1) 的结构
        self.W = nn.Linear(input_dim, 1)

    def forward(self, batch_rep):
        """
        input:
            batch_rep: (Batch, Length, Dim)
        return:
            vector: (Batch, Dim)
        """
        # 1. 计算注意力分数 (Batch, Length, 1)
        # 模型自行判断每一帧的重要性
        attn_logits = self.W(batch_rep)

        # 2. 归一化为概率分布 (Batch, Length, 1)
        # 对 Length 维度 (dim=1) 做 Softmax
        attn_weights = torch.softmax(attn_logits, dim=1)

        # 3. 加权求和
        # (Batch, Length, Dim) * (Batch, Length, 1) -> (Batch, Length, Dim)
        # sum(dim=1) -> (Batch, Dim)
        x = torch.sum(batch_rep * attn_weights, dim=1)
...
#         input:
#             batch_rep: (Batch, Length, Dim)
#             pad_mask: (Batch, Length)  True 表示是 Padding，需要被忽略
#         """
#         # (Batch, Length, 1)
...
#             # 将 Padding 位置的分数设为极小值 (-1e9)
#             att_logits = att_logits.masked_fill(pad_mask, -1e9)
#
#         # (Batch, Length)
#         att_w = torch.softmax(att_logits, dim=1).unsqueeze(-1)
#
#         # 加权求和
#         x = torch.sum(batch_rep * att_w, dim=1)
#         return x
...
def main(
    ...
    for feat_paths, mels in tqdm(dataloader):
        with torch.no_grad():
            ...
            # 5. 并行预测
            outs = model(batch_mels, labels=None)
